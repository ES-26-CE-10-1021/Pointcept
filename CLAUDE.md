...
- **Frozen Utonia pre-encoder**: `PTv3m3PreEncoder` wraps `PT-v3m3` (Utonia) as a pretrained VFM. Loads the HuggingFace checkpoint via `third_party.utonia.utonia.model.load(ckpt_only=True)` in a DDP-safe rank-0 + barrier pattern, zero-pads missing modalities (rgb / normal) on-device into the 9-ch input, and detaches + `requires_grad_(True)`'s the output leaf so downstream 3DETR layers can still backprop. Supports both encoder-only (`enc_mode=True`, default) and full U-Net (`enc_mode=False`) operation, and optional FPS downsampling (`npoint=K`) that returns a fixed-length `(xyz, features, inds)` tuple matching `PointnetSAPreEncoder`. Freeze behaviour is a single enum `freeze_backbone`: `"full"` (embedding + enc + dec all frozen, eval mode, no_grad forward — the standard frozen-VFM setup), `"enc"` (encoder frozen, decoder trainable — for future partial fine-tune configs), or `"none"` (full gradient flow). The full backbone hyperparameter set (channels, depths, `dec_*` for U-Net, etc.) is driven by the checkpoint config; the pointcept config only specifies `pretrained`, `grid_size`, `enc_mode`, `npoint`, and `freeze_backbone`.
- **`pre_encoder=None`**: skips PointNet++ SA; set `input_feature_dim` to match input channels beyond XYZ
- **Configs**: `det-3detr-v0m1-0-scannet.py` (native 3DETR baseline), `det-3detr-v1m1-0-scannet.py` (PointNet++ SA encoder), `det-3detr-v2m1-0-scannet.py` (PTv3 + Identity encoder), `det-3detr-v3m1-0-scannet.py` (PTv3 + Vanilla Transformer encoder), `det-3detr-v3m1-1-scannet.py` (PTv3 + FPS + Vanilla Transformer encoder), `det-3detr-v4m1-0-scannet.py` (PTv3 U-Net + Identity encoder), `det-3detr-utonia-v1m1-0-scannet.py` (Frozen Utonia PT-v3m3 + Vanilla Transformer encoder), `det-3detr-utonia-v2m1-0-scannet.py` (… + Utonia-canonical preprocessing), `det-3detr-utonia-v3m1-0-scannet.py` (… + RGB input), `det-3detr-utonia-v4m1-0-scannet.py` (Frozen Utonia encoder-only + FPS 2048), `det-3detr-utonia-v4m2-0-scannet.py` (Frozen Utonia full U-Net + FPS 2048)
