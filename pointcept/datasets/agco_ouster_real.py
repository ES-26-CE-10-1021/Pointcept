...
class OusterRealDataset(DefaultDataset):
    """
    Splits:
    - train: all from static_2_ouster/split_2
    - val:   first half from static_1_ouster/split_1
    - test:  second half from static_1_ouster/split_1
    """
    ...
    def get_data_list(self):
        ...
        for split in split_list:
            if split == "train":
                split_dir = os.path.join(self.data_root, "static_2_ouster", "split_2")
                scenes = sorted(d for d in glob.glob(os.path.join(split_dir, "scene_*")) if os.path.isdir(d))
                data_list.extend(scenes)
            elif split in ["val", "test"]:
                split_dir = os.path.join(self.data_root, "static_1_ouster", "split_1")
                scenes = sorted(d for d in glob.glob(os.path.join(split_dir, "scene_*")) if os.path.isdir(d))
                mid = len(scenes) // 2
