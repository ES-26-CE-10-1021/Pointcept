"""
3DETR on AGCO — v2m4-0-3cls-color-ouster: Utonia + FPS 2048 + SUN-like loss + RGB (ouster).

Differs from `det-3detr-v2m4-0-3cls-agco-ouster.py` only by `use_color=True`
in train/val/test. Loads per-point RGB from `<sensor>/color/<ts>.npy`; the
dataset's `utonia_preprocess=True` block right-pads the 6-channel
`[xyz, rgb]` tensor to 9 with zero normals, matching the Utonia
checkpoint's expected `[xyz, rgb, normal]` layout.
...
Usage:
  sh scripts/train.sh -d agco -c det-3detr-v2m4-0-3cls-agco-color-ouster -n v2m4_color_ouster -g 2
"""
