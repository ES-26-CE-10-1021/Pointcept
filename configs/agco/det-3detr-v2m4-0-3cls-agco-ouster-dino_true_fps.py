"""
3DETR on AGCO — v2m4-0-3cls-ouster-dino_true_fps: Utonia (enc_finetune) + FPS 2048 + DINO fusion (ouster).

Architecture:
  PTv3m3PreEncoderWithDinoTrueFps (pretrained Utonia, enc_finetune, npoint=2048 FPS)
  + DinoInjectionEncoder wrapping VanillaTransformerEncoder3DETR(576d, 3L)
  + TransformerDecoder3DETR(256d, 8L). num_queries=32,
  center_offset_normalized=True, projection_norm="ln", max_num_obj=16.

"True FPS" variant: instead of mean-pooling DINO through PTv3's voxel
downsampling chain, each FPS-selected center gathers the DINO of the
single nearest raw input point via torch_cluster.knn(k=1) — same semantics
as v1m3-dino, contrast to v2m4-dino's voxel-pooled fusion.

Dataset:
  AgcoBBoxDinoV1, sensors=["ouster"], num_points=40_000, 3-class, normal
...
Usage:
  sh scripts/train.sh -d agco -c det-3detr-v2m4-0-3cls-agco-ouster-dino_true_fps -n v2m4_ouster_dino_true_fps -g 1
"""
...
model = dict(
    ...
    pre_encoder=dict(
        type="PTv3m3PreEncoderWithDinoTrueFps",
        pretrained="utonia",
        grid_size=0.05,
        enc_mode=True,
        npoint=2048,
