"""
3DETR on AGCO — v1m3-0-3cls-color-ouster: PointNet++ + AGCO knobs + SUN-like loss + RGB (ouster).

Differs from `det-3detr-v1m3-0-3cls-agco-ouster.py` only by `use_color=True`
in train/val/test and `PointnetSAPreEncoder.mlp_dims[0]=3` (3 extra channels
beyond XYZ). Loads per-point RGB from `<sensor>/color/<ts>.npy`.
...
Usage:
  sh scripts/train.sh -d agco -c det-3detr-v1m3-0-3cls-agco-color-ouster -n v1m3_color_sunloss_ouster -g 2
"""
