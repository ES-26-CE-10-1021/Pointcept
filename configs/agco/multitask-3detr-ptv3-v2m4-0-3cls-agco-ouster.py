"""
Multi-task PTv3 + 3DETR on AGCO (semseg + detection) — v2m4 ouster variant.

Mimics ``det-3detr-v2m4-0-3cls-agco-ouster.py`` (Utonia ``enc_finetune`` +
FPS 2048 + Vanilla encoder + SUN-like loss) and adds the semantic
segmentation branch from the multi-task scaffold. Architecture:

    input (B, N, 3) ─► Utonia embedding ─► Utonia encoder (enc_finetune) ─► bottleneck ─┐
                                                                                        │
                                ┌───────────────────────────────────────────────────────┘
                                │
...
The Utonia distributed checkpoint is encoder-only, so the multi-task
backbone bolts a randomly-initialised decoder onto the pretrained encoder
(``dec_*`` kwargs below). Per CLAUDE.md, ``_load_pretrained_state`` will
log the expected dec-missing keys at INFO. The encoder is frozen except
for the last encoder stage (``freeze_backbone="enc_finetune"``); the fresh
decoder is fully trainable.

Detection branch FPS-subsamples the bottleneck to 2048 tokens and runs the
Vanilla transformer encoder at the Utonia bottleneck width
(``UTONIA_ENC_DIM=576``). ``center_offset_normalized=True`` so center
offsets are scaled by the scene's ``fixed_pc_dims``.
...
Usage:
    sh scripts/train.sh -d agco -c multitask-3detr-ptv3-v2m4-0-3cls-agco-ouster \\
        -n mt_v2m4_ouster_720ep -g 1
"""
