"""
3DETR on AGCO — v5m2-0-3cls-ouster: Frozen Utonia + Vanilla encoder
("Frozen Utonia + vanilla transformer head", ouster).

Foundation-model evaluation variant. The Utonia backbone is fully frozen
(``freeze_backbone="enc"``); on top of that the standard 3-layer 3DETR
transformer encoder is restored (the v5m1 variant uses an identity
pass-through instead). This isolates the contribution of the 3DETR
transformer encoder on top of frozen Utonia features — directly
comparable against v5m1-0 (no encoder) and v2m4-0 (enc_finetune,
trainable last stage).

Architecture:
  PTv3m3PreEncoder (pretrained Utonia, frozen, npoint=2048 FPS)
...
Usage:
  sh scripts/train.sh -d agco -c det-3detr-v5m2-0-3cls-agco-ouster -n v5m2_ouster -g 2
"""
