"""
Seg-only PTv3 (Utonia) on AGCO — v5m4 ouster variant.

Identical to ``semseg-ptv3-v2m4-0-3cls-agco-ouster.py`` except the Utonia
backbone is **fully frozen** (``freeze_backbone="enc"``) instead of
``"enc_finetune"``. The fresh PT-v3m1-style decoder bolted onto the
encoder-only Utonia checkpoint remains fully trainable, as does the
semseg head.

This is the seg-only counterpart to ``multitask-3detr-ptv3-v5m4-0-3cls-agco-ouster.py``
and ``det-3detr-v5m2-0-3cls-agco-ouster.py``, completing the fully-frozen
Utonia three-way comparison:
  det-only (v5m2) ↔ multi-task (multitask v5m4) ↔ seg-only (this config).
...
Usage:
    sh scripts/train.sh -d agco -c semseg-ptv3-v5m4-0-3cls-agco-ouster \\
        -n seg_v5m4_ouster_720ep -g 1
"""
