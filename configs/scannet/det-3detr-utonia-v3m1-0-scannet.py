"""
3DETR on ScanNet — Utonia (PT-v3m3) frozen pre-encoder, v3.

v3 = v2 + use_color=True with Utonia-style ``rgb/255`` normalization.
Normals are still absent → normal slots zero-padded on-device by
``PTv3m3PreEncoder._build_padded_feat``.

Differences from ``det-3detr-utonia-v1m1-0-scannet.py``:
  - ``pre_encoder.grid_size = 0.01``  (matches voxel topology after 0.5× scale)
  - ``data.*.utonia_preprocess = True``  (scale 0.5 + CenterShift + rgb/255)
  - ``data.*.use_color = True``  (RGB forwarded to PT-v3m3 RGB slots)

Differences from ``det-3detr-utonia-v2m1-0-scannet.py``:
  - ``data.*.use_color = True``

Usage:
    sh scripts/train.sh -d scannet -c det-3detr-utonia-v3m1-0-scannet -n my_exp -g 4
"""
