"""
3DETR on ScanNet — Utonia frozen encoder + fresh trainable decoder + FPS, v5m2-1.

v5m2-1 = v5m2-0 with FPS(2048) after the U-Net, producing fixed-length
tokens (no padding mask). Token-budget counterpart to v5m2-0.

As with v5m2-0: the Utonia checkpoint is encoder-only, so the decoder is
**randomly initialized** and trains from scratch. ``_load_pretrained_state``
logs every ``dec.*`` key as expected-missing.

Usage:
    sh scripts/train.sh -d scannet -c det-3detr-utonia-v5m2-1-scannet -n my_exp -g 4
"""
