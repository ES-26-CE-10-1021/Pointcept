...
v3m1-1 + COLOR: PTv3 pre-encoder + FPS downsampling (2048 pts) +
        VanillaTransformerEncoder + 3DETR decoder, with RGB input.

Differs from `det-3detr-v3m1-1-scannet.py` only by `use_color=True` and
`PTv3PreEncoder.in_channels=6` (xyz + rgb).
...
Usage:
    sh scripts/train.sh -d scannet -c det-3detr-v3m1-1-scannet-color -n 3detr_ptv3_fps_color -g 4
