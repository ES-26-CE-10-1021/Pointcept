"""
3DETR on ScanNet — Utonia encoder, last stage fine-tuned + FPS, NO COLOR.

Differs from `det-3detr-utonia-v5m1-1-scannet.py` only by `use_color=False`
in train/val/test. The Utonia (PT-v3m3) backbone still receives 9-channel
input — `utonia_preprocess=True` keeps the per-sample normalization, and
`PTv3m3PreEncoder._build_padded_feat` zero-pads RGB (chs 3..5) and the
normal slot (chs 6..8) on-device to reach `embedding.in_channels=9`.

This is the fair no-color baseline against the v0 / v3 ScanNet runs in
the thesis ScanNet triple.

Usage:
    sh scripts/train.sh -d scannet -c det-3detr-utonia-v5m1-1-scannet-nocolor -n utonia_v5m1_1_nocolor -g 4
"""
